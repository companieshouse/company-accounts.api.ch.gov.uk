...
@Component
public class CurrentPeriodInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request,
        ...
        if (request.getMethod().equalsIgnoreCase("POST")
                && request.getRequestURI().endsWith("current-period")) {
            return true;
        }
        ...
        if (transaction == null) {
             LOGGER.errorRequest(request, "CurrentPeriodInterceptor error: No transaction in request session",
                     debugMap);
        ...
        if (smallFull == null) {
              LOGGER.errorRequest(request, "CurrentPeriodInterceptor error: No company account in request session",
                      debugMap);
            response.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
            return false;
        }
        ...
        String smallFullLink = smallFull.getLinks().get(SmallFullLinkType.CURRENT_PERIOD.getLink());
